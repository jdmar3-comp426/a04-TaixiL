...
app.get("/app/user/:id", (req, res) => {
	...
	const stmt = db.prepare("SELECT * FROM userinfo WHERE id = ?").get(2);
	res.status(200).json(stmt);

});
// UPDATE a single user (HTTP method PATCH) at endpoint /app/update/user/:id
app.patch("/app/update/user/:id", (req, res) => {
	const stmt = db.prepare("UPDATE userinfo SET user = COALESCE(?,user), pass = COALESCE(?,pass) WHERE id = ?").run("user", "pass", 2);
